    ...
    child: Container(
      ...
      child: Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            if (game.whoScored != PlayerColor.noPlayer) whoScoredText,
            Text('Red Team: ${game.redscore}',
                style: const TextStyle(fontSize: 24, color: Colors.black)),
            const Text('', style: TextStyle(fontSize: 12)),
            Text('Blue Team: ${game.bluescore}',
                style: const TextStyle(fontSize: 24, color: Colors.black)),
            const Text('', style: TextStyle(fontSize: 24)),
            GestureDetector(
                onTap: () {
...
Widget winnerMenuBuilder(BuildContext buildContext, AirHockeyGame game) {
  ...
  if (game.theWinner == PlayerColor.redPlayer) {
    winnerText = const Text('Team Red Wins!',
        style: TextStyle(fontSize: 24, color: Colors.black));
  } else {
    winnerText = const Text('Team Blue Wins!',
        style: TextStyle(fontSize: 24, color: Colors.black));
  }
  return Center(
    child: Container(
